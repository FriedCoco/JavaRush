...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        var aDayOfWeek = dateTime.getDayOfWeek();
        var aTimeOfDay = dateTime.getHour();
        if ((aDayOfWeek == DayOfWeek.FRIDAY) && (aTimeOfDay >= 22)) {
            return true;
        } else if (aDayOfWeek == DayOfWeek.SATURDAY) {
            return true;
        } else if ((aDayOfWeek == DayOfWeek.SUNDAY) && (aTimeOfDay < 23)) {
            return true;
        } else {
            return false;
        }
